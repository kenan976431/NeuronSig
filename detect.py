...
# %%
model = load_model('/data0/jinhaibo/lixiaohao/adv_cifar10/Vgg16/VGG16.h5')
model.summary()
...
# advs = np.load('/home/lxh/lixiaohao/code/dataset/cifar10/VGG19/adv/Square/x_train_adv.npy')
# x_diff = advs[2] - x_train[2]

# x_diff_1 = x_diff.reshape[-1]
# plt.imshow(x_diff)
# plt.show()
...
benign_features = []
adv_features = []
# path = '/public/lixiaohao/features4demo/CIFAR10/VGG19/FGSM/Adv'
for i in range(10):
    path_Benign = '/data0/jinhaibo/lixiaohao/adv_cifar10/Vgg16/clean/' + str(i) + '.npy'
    benign_features.extend(np.load(path_Benign))
    path_adv = '/data0/jinhaibo/lixiaohao/adv_cifar10/Vgg16/FGSM/' + str(i) + '.npy'
    adv_features.extend(np.load(path_adv))
# data = np.array(benign_features)
# print(data.shape)
# %%
test_features = []
path = '/data0/jinhaibo/lixiaohao/adv_cifar10/Vgg16/JSMA/'
for i in range(10):
...
#     if i == 0:
#         x_to_train_hhh = x_to_train[i:i+1]
#         y_to_train_hhh = y_to_train_one[i:i+1]
#     else:
#         if math.isnan(np.max(x_to_train[i:i + 1])):
#             continue
#         else:
#             x_to_train_hhh = np.vstack((x_to_train_hhh, x_to_train[i:i+1]))
#             y_to_train_hhh = np.vstack((y_to_train_hhh, y_to_train_one[i:i + 1]))
# %%
...
# model_detect.save('/home/lxh/lixiaohao/features4demo/CIFAR10/VGG19/cifar_fgsm_vgg19.h5')
# %%ACC score
